...
void MixingModel::Add_NeutralBd_meas(){
  ...
  if (comb == 1){   // When treating separately from the Bs counterparts
...
void MixingModel::Add_NeutralBs_meas(){
  ...
  if (comb == 2){  // If treating it separately from the Bd counterpart
  ...
  else if(comb == 3){ // this is if you are considering all the observables because Bd and Bs are correlated
...
void MixingModel::Add_time_dependent_Dmeas(){
  ...
  */
  ...
  //Observables 1:
  meas.insert(pair<string,dato>("UID29", dato(-0.0001,0.00011,0.00003)));// DY // Combining all the LHCb results
...
*/
  ...
  //Observables 4:
  CorrData.clear();
  CorrData.push_back(dato(0.0040,0.0004,0.0002));//xcp
  CorrData.push_back(dato(0.0055,0.0012,0.0006));//ycp
...
void MixingModel::Add_old_meas(){
  ...
  CorrData.push_back(dato(-0.022e-2, 0.030e-2, 0.021e-2)); // xp2plus_exp_babar_kp
  CorrData.push_back(dato(0.97e-2, 0.44e-2 , 0.31e-2)); // ypplus_exp_babar_kp
...
void MixingModel::DefineHistograms(){
  ...
  int j = 0;
  for(int i = 0; i < nVarab.size(); i++){ // looping over all the names of the variables
...
double MixingModel::Calculate_neutralBdobservables(){
  ...
  if(comb == 1){ // If treating it seprately from the Bs counterpart
...
double MixingModel::Calculate_neutralBsobservables(){
  ...
  if(comb == 2){ // If treating it separately from Bd counterpart
  ...
  else if (comb == 3){ // When using also the Bd counterpart
